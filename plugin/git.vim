...
let g:GIT_diffguitool = get(g:, 'GIT_diffguitool', 0)
...
function! GIT_CompleteGit(L, C, P)
    ...
    elseif a:L =~ '^-' && len(filter(copy(l:ex), "v:val !~ '^-'")) < 3
        let l:list = systemlist('git help '.(l:ex[1] =~ '^-' ? 'git' : l:ex[1])."|sed -E -n '".
                    \ 's/^ +(-{1,2}\w[-a-zA-Z]*).*/\1/p'."'|grep -e '".a:L."'")
    elseif len(l:ex) == 1 || (len(l:ex) == 2 && a:L != '')
        let l:list = systemlist("man git|sed -E -n '".'s/^ +git-([-a-zA-Z]*).*/\1/p'."'|grep -e '".a:L."'")
    elseif l:ex[1] !~ '^-' && ((len(l:ex) == 2 && a:L == '') || (len(l:ex) == 3 && a:L != ''))
        let l:list = systemlist('man git '.l:ex[1]."|sed -E -n '".
